...
char* Eventinfo_to_jsonstr(const Eventinfo* lf, bool force_full_log, OSList* list_msg)
{
    ...
    if (lf->generated_rule)
    {
        ...
        if (lf->generated_rule->mitre_technique_id && lf->generated_rule->mitre_tactic_id)
        {
            ...
            for (i = 0; lf->generated_rule->mitre_technique_id[i] != NULL; i++)
            {
                ...
                {
                    smwarn(list_msg,
                           "Mitre Technique ID '%s' not found in database.",
                ...
                else
                {
                    ...
                    if (tactic_exist == TRUE)
                    {
                        ...
                        if (!inarray)
                        {
                            ...
                            cJSON_AddItemToArray(mitre_technique_array,
                                                 cJSON_CreateString(data_technique->technique_name));
                        }
                    }
                    else
                    {
                        smwarn(list_msg,
                               "Mitre Tactic ID '%s' is not a tactic of '%s'.",
    ...
    if (lf->decoder_info->name != NULL && strncmp(lf->decoder_info->name, "syscheck_", 9) == 0)
    {
        ...
        if (Config.decoder_order_size > FIM_DIFF && lf->fields[FIM_DIFF].value &&
            strcmp(lf->fields[FIM_DIFF].value, "0"))
        {
...
void add_json_attrs(const char* attrs_str, cJSON* file_diff, char after)
{
    ...
    if (attrs)
    {
        cJSON_AddItemToObject(file_diff, after ? "attrs_after" : "attrs_before", attrs);
    }
    else
    {
        merror("The %s file attributes could not be added to the JSON alert.", after ? "new" : "before");
    }
}
